...
const ArtCard = ({ data, artKey, isLight }) => {
    ...
    return (
        <>
            <div className={`md:w-80 lg:w-[22rem] mb-12 hover:shadow-[0_0px_25px_-5px_rgba(158,158,158,0.4)] transition border-2 border-[${reverseColor}] rounded-lg`}>
                <div className="flex flex-col h-full overflow-hidden ">
                    ...
                    <div className="p-5 flex flex-col h-full justify-between">
                        ...
                        <div>
                            <h1 className={`rajdhani-semibold title-font text-xl font-medium ${textColor} mb-2`}>{projectTitle}</h1>
                            <p className={`rajdhani-medium leading-relaxed mb-4 text-base ${textColor}`}>{description}</p>
                        </div>


                        <div className="flex items-center justify-between flex-wrap ">
                            <div className={"flex flex-wrap flex-row gap-1"}>
                                {tags.map((tag) => <p className={`rajdhani-semibold py-[2px] p-1.5 border-2 border-[${reverseColor}] rounded text-xs ${textColor} bg-transparent`}>{tag.name}</p>)}
                            </div>
